...
#########################################################################################################
        ...
        #Mask
        filter_frame = cv2.inRange(self.image, np.array([0, 0, 0]), np.array([255, 53, 240]))
        #erode twice
        erode_frame = cv2.erode(filter_frame, element)
        erode_frame = cv2.erode(erode_frame, element)
         
        #dilate
        dilate_frame = cv2.dilate(erode_frame, element2)
        thresh = cv2.dilate(dilate_frame, element2)        
